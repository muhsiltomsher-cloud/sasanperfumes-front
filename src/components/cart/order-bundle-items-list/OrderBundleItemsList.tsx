...
export function OrderBundleItemsList({ item, locale, compact = false, showPrices = true }: OrderBundleItemsListProps) {
  ...
  const fixedItems = bundleItems.filter(bi => bi.is_fixed);
  const regularItems = bundleItems.filter(bi => !bi.is_addon && !bi.is_free && !bi.is_fixed);
  const addonItems = bundleItems.filter(bi => bi.is_addon && !bi.is_free && !bi.is_fixed);
  const freeItems = bundleItems.filter(bi => bi.is_free && !bi.is_fixed);
  ...
  if (compact) {
    return (
      <div className="mt-1 text-xs text-gray-500">
        {fixedItems.length > 0 && (
          <>
            <span className="font-medium">{isRTL ? "مشمول:" : "Included:"}</span>
            <ul className="mt-0.5 space-y-0.5">
              {fixedItems.map((bi, idx) => {
                const baseQty = bi.quantity || 1;
                const displayQty = baseQty * orderQuantity;
                return (
                  <li key={`fixed-${bi.product_id}-${idx}`} className="flex items-center justify-between gap-1">
                    <span className="flex items-center gap-1">
                      <span className="h-1 w-1 rounded-full bg-gray-400 flex-shrink-0"></span>
                      <span className="truncate">{bi.name || `Product #${bi.product_id}`}</span>
                      <span className="text-gray-400 flex-shrink-0">x{displayQty}</span>
                    </span>
                    <span className="text-xs text-gray-400 flex-shrink-0">{isRTL ? "مشمول" : "Included"}</span>
                  </li>
                );
              })}
            </ul>
              ...
              {regularItems.map((bi, idx) => {
                ...
                return (
                  <li key={`${bi.product_id}-${idx}`} className="flex items-center justify-between gap-1">
                    <span className="flex items-center gap-1">
                      <span className="h-1 w-1 rounded-full bg-brand-gold flex-shrink-0"></span>
                      <span className="truncate">{bi.name || `Product #${bi.product_id}`}</span>
                      <span className="text-gray-400 flex-shrink-0">x{displayQty}</span>
                    </span>
        ...
        {addonItems.length > 0 && (
          <>
            <div className="mt-1 pt-1 border-t border-gray-100">
              <span className="font-medium text-brand-gold">{isRTL ? "الإضافات:" : "Add-ons:"}</span>
            </div>
            <ul className="mt-0.5 space-y-0.5">
                ...
                return (
                  <li key={`addon-${bi.product_id}-${idx}`} className="flex items-center justify-between gap-1">
                    <span className="flex items-center gap-1">
                      <span className="h-1 w-1 rounded-full bg-brand-gold flex-shrink-0"></span>
                      <span className="truncate">{bi.name || `Product #${bi.product_id}`}</span>
                      <span className="text-brand-gold flex-shrink-0">x{displayQty}</span>
                    </span>
                    {showPrices && displayPrice !== undefined && displayPrice > 0 && (
                      <FormattedPrice price={displayPrice} className="text-xs text-brand-gold flex-shrink-0" iconSize="xs" />
                    )}
                  </li>
                );
              })}
            ...
            {showPrices && addonItemsTotal > 0 && (
              <div className="mt-0.5 flex items-center justify-between text-brand-gold">
                <span className="text-[10px]">{isRTL ? "مجموع الإضافات:" : "Add-ons Total:"}</span>
                <FormattedPrice price={addonItemsTotal} className="text-xs text-brand-gold flex-shrink-0" iconSize="xs" />
              </div>
            )}
          </>
        )}
        ...
        {showPrices && !isFixedPricing && bundleTotal > 0 && (
          <div className="mt-1 flex items-center justify-between border-t border-gray-200 pt-1">
            ...
            <FormattedPrice price={bundleTotal} className="text-xs font-medium text-brand-gold" iconSize="xs" />
          </div>
        )}
        ...
        {showPrices && (isFixedPricing || (boxPrice !== null && boxPrice > 0)) && (
          <div className="mt-1 flex items-center justify-between border-t border-gray-200 pt-1">
            ...
            <FormattedPrice price={totalPrice} className="text-xs font-medium text-brand-gold" iconSize="xs" />
          </div>
        )}
      </div>
    );
  ...
  return (
    <div className="mt-2 rounded-md bg-gray-50 p-3">
      {fixedItems.length > 0 && (
        <>
          <p className="mb-2 text-xs font-medium text-gray-500">{isRTL ? "مشمول:" : "Included:"}</p>
          <ul className="space-y-1.5">
            {fixedItems.map((bi, idx) => {
              const baseQty = bi.quantity || 1;
              const displayQty = baseQty * orderQuantity;
              return (
                <li key={`fixed-${bi.product_id}-${idx}`} className="flex items-center justify-between gap-2 text-xs">
                  <span className="flex items-center gap-1.5 text-gray-500">
                    <span className="h-1.5 w-1.5 rounded-full bg-gray-400 flex-shrink-0"></span>
              ...
              return (
                <li key={`${bi.product_id}-${idx}`} className="flex items-center justify-between gap-2 text-xs">
                  <span className="flex items-center gap-1.5 text-gray-600">
                    <span className="h-1.5 w-1.5 rounded-full bg-brand-gold flex-shrink-0"></span>
                    <span>{bi.name || `Product #${bi.product_id}`}</span>
                    <span className="text-gray-400">x{displayQty}</span>
                  </span>
                  {showPrices && displayPrice !== undefined && displayPrice > 0 && (
      ...
      {addonItems.length > 0 && (
        <>
          <div className="mt-3 pt-2 border-t border-gray-200">
            <p className="mb-2 text-xs font-medium text-brand-gold">{isRTL ? "الإضافات:" : "Add-ons:"}</p>
          </div>
          <ul className="space-y-1.5">
            {addonItems.map((bi, idx) => {
              const basePrice = typeof bi.price === "string" ? parseFloat(bi.price) : bi.price;
              ...
              return (
                <li key={`addon-${bi.product_id}-${idx}`} className="flex items-center justify-between gap-2 text-xs">
                  <span className="flex items-center gap-1.5 text-brand-primary">
                    <span className="h-1.5 w-1.5 rounded-full bg-brand-gold flex-shrink-0"></span>
                    <span>{bi.name || `Product #${bi.product_id}`}</span>
                    <span className="text-brand-gold">x{displayQty}</span>
                  </span>
                  {showPrices && displayPrice !== undefined && displayPrice > 0 && (
                    <FormattedPrice price={displayPrice} className="text-xs text-brand-gold flex-shrink-0" iconSize="xs" />
                  )}
          ...
          {showPrices && addonItemsTotal > 0 && (
            <div className="mt-1.5 flex items-center justify-between text-brand-gold">
      ...
      {showPrices && !isFixedPricing && bundleTotal > 0 && (
        <div className="mt-2 flex items-center justify-between border-t border-gray-200 pt-2">
          ...
          <FormattedPrice price={bundleTotal} className="text-sm font-semibold text-brand-gold" iconSize="xs" />
        </div>
      )}
      ...
      {showPrices && (isFixedPricing || (boxPrice !== null && boxPrice > 0)) && (
        <div className="mt-1 flex items-center justify-between border-t border-gray-200 pt-2">
          ...
          <FormattedPrice price={totalPrice} className="text-sm font-semibold text-brand-gold" iconSize="xs" />
        </div>
      )}
    </div>
  );
