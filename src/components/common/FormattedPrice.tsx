    ...
    return (
      <span className={cn("inline-flex items-center gap-1", strikethrough && "line-through", className)}>
        {isAED ? (
          ...
          <span className="![font-size:inherit]">{currencyInfo.code}</span>
        )}
        <span className="![font-size:inherit]">{formattedNumber}</span>
      </span>
    );
  }
  ...
  return (
    <span className={cn("inline-flex items-center gap-0.5", strikethrough && "line-through", className)}>
      {isAED ? (
        ...
        <span className="![font-size:inherit]">{currencyInfo.symbol}</span>
      )}
      <span className="![font-size:inherit]">{formattedNumber}</span>
    </span>
  );
